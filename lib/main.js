    ...
    update: function(time, delta) {
        if(this.start) {
            for(var i=0; i<9; i++) {
                ...
                if(time > this.now + 2000 & (y_ % 200) < -190) {
                    let yint_ = Math.round(y_-10);
            ...
            if(checkEnd(this)) {
                ...
                if(matched.length > 0) {
                    ...
                    this.coin += matched.filter(m => m == true).length*this.bet*2;
...
function setStart(game, bet) {
    let scene = game.scene.scenes[0];
    scene.start = true;
    scene.now = scene.time.now;
    scene.coin -= bet;
    scene.bet = bet;
    updatedom(scene.start);
    if(scene.lines == null) {
        scene.lines = [];
    } else {
...
function updatedom(mode) {
    ...
    const goal = 100;
    const ratio = coin / goal * 100;
    coin_bar.style = `width:${ratio}%`;

    const hit_max = coin >= goal;
    if(hit_max) {
        alert(`축하합니다~ 승리!`);
    }
    const btn1 = document.querySelector("button#lever1");
    const btn2 = document.querySelector("button#lever2");
    const btn4 = document.querySelector("button#lever4");
    btn1.disabled = btn_mode;
    btn2.disabled = btn_mode;
    btn4.disabled = btn_mode;
}
